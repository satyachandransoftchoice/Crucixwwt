...
export class DiscordAlerter {
  ...
  async _handleCommand(interaction) {
    // Reject interactions that didn't originate from the configured guild/channel.
    ...
    console.log(`[Audit] Discord command /${name} | user=${interaction.user?.id} tag=${interaction.user?.tag} | guild=${interaction.guildId || 'DM'} channel=${interaction.channelId} | ${new Date().toISOString()}`);
