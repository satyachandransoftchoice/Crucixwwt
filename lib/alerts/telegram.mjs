...
import { escapeMd } from '../sanitize.mjs';
...
export class TelegramAlerter {
  constructor({ botToken, chatId, allowedUserId = null }) {
    this.botToken = botToken;
    this.chatId = chatId;
    // Optional: restrict bot commands to a single Telegram user ID (TELEGRAM_ALLOWED_USER_ID).
    // When null, any member of the configured chat can invoke commands.
    this.allowedUserId = allowedUserId ? String(allowedUserId) : null;
  ...
  async _pollUpdates() {
    try {
      ...
      for (const update of data.result) {
        ...
        // When an allowedUserId is configured, reject commands from any other sender.
        if (this.allowedUserId && String(msg.from?.id) !== this.allowedUserId) continue;
  ...
  async _handleMessage(msg) {
    ...
    console.log(`[Audit] Telegram command ${command} | user=${msg.from?.id} username=${msg.from?.username || 'none'} | chat=${msg.chat?.id} | ${new Date().toISOString()}`);
...
// Re-export so server.mjs can import escapeMd from this module (existing API).
export { escapeMd } from '../sanitize.mjs';
